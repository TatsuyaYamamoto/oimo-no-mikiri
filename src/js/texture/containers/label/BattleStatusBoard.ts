...
class BattleStatusBoard extends Container {
    ...
    constructor(width: number, height: number) {
        ...
        this._onePlayerWinsLabel = new Text(t(StringIds.LABEL_WINS), winsLabelStyle);
        ...
        this._twoPlayerWinsLabel = new Text(t(StringIds.LABEL_WINS), winsLabelStyle);
        ...
        this._battleLeftLabel = new Text(t(StringIds.LABEL_BATTLE_LEFT), battleLeftLabelStyle);
    ...
    set battleLeft(battleLeft: number) {
        if (!Number.isInteger(battleLeft)) {
            throw new Error(`BattleLeft should be integer. Provided value is ${battleLeft}`);
        }
